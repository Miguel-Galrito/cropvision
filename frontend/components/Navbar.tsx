      ...
      <div className="flex items-center space-x-2 sm:space-x-3 shrink-0">
        ...
        {onLanguageChange && (
          <button
            ...
            className="flex items-center space-x-1 px-2.5 py-1.5 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-700 text-xs font-bold text-slate-300 hover:text-white transition-all shadow-sm"
        ...
        {onOpenParcelUploader && (
          <button
            ...
            title={lang === 'en' ? 'Upload field boundaries (Shapefile, GeoJSON, KML)' : 'Carregar limites cadastrais (Shapefile, GeoJSON ou KML)'}
          >
            <UploadCloud className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
            <span className="hidden sm:inline">{t.sigPolygon}</span>
        ...
        {/* Pricing / Monetization Button (Professional Name) */}
        {onOpenPricing && (
          <button
